...
def main():
    ...
    try:
        ...
        try:
            ...
            model_count = sum(len(category_models) for category_models in models.values())
            print(f"✅ 成功加载 {model_count} 个模型 / Successfully loaded {model_count} models")
            ...
            for category, category_models in models.items():
                if category_models:
                    ...
                    for model in category_models[:2]:  # 只显示前2个 / Only show first 2
                        display_name = model.get('displayName', model.get('modelId', 'Unknown'))
                        print(f"    - {display_name}")
                    if len(category_models) > 2:
                        print(f"    ... 还有 {len(category_models) - 2} 个模型 / ... and {len(category_models) - 2} more")
