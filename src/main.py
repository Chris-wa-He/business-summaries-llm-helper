...
def parse_arguments():
    ...
    parser.add_argument(
        "--share", action="store_true", help="创建公共链接 / Create public link"
    )

    parser.add_argument(
        "--debug", action="store_true", help="启用调试模式 / Enable debug mode"
    )
...
def main():
    ...
    try:
        ...
        if not is_initialized:
            logger.error(
                f"应用控制器初始化失败: {status_message} / App controller initialization failed: {status_message}"
            )
            sys.exit(1)

        logger.info(
            f"应用控制器初始化成功: {status_message} / App controller initialized successfully: {status_message}"
        )
        ...
        logger.info(
            f"启动Web界面: http://{args.host}:{args.port} / Launching web interface: http://{args.host}:{args.port}"
        )
