...
class HistoryProcessor:
    ...
    def load_history_files(self) -> List[Dict[str, str]]:
        ...
        try:
            if not self.history_folder.exists():
                self.logger.warning(
                    f"历史参考文件夹不存在: {self.history_folder} / History folder not found: {self.history_folder}"
                )
            ...
            for file_path in self._get_supported_files():
                try:
                    ...
                    if content.strip():  # 只添加非空文件 / Only add non-empty files
                        ...
                        self.logger.debug(
                            f"成功读取历史文件: {file_path} / Successfully read history file: {file_path}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"读取文件失败 {file_path}: {e} / Failed to read file {file_path}: {e}"
                    )
                    continue

            self.logger.info(
                f"成功加载 {len(history_files)} 个历史文件 / Successfully loaded {len(history_files)} history files"
            )
            return history_files

        except Exception as e:
            raise HistoryProcessingError(
                f"加载历史文件失败: {e} / Failed to load history files: {e}"
            )
        ...
        for category, category_files in categorized_files.items():
            ...
            for file_info in category_files:
                ...
                section_content.append(
                    f"### 文件: {file_info['name']} / File: {file_info['name']}"
                )
    ...
    def filter_relevant_history(self, case_input: str, history: str) -> str:
        ...
        filtered_content.append(
            "## 相关历史参考信息 / Relevant Historical Reference Information\n"
        )

        for section, score in relevant_sections[
            :5
        ]:  # 最多返回5个最相关的段落 / Return at most 5 most relevant paragraphs
        ...
        try:
            ...
            for encoding in ["gbk", "gb2312", "latin1"]:
                try:
                    ...
                    continue
            raise HistoryProcessingError(
                f"无法解码文件: {file_path} / Cannot decode file: {file_path}"
            )
    ...
    def _get_file_category(self, file_path: Path) -> str:
        ...
        if len(relative_path.parts) > 1:
            return relative_path.parts[
                0
            ]  # 使用第一级目录作为类别 / Use first level directory as category
        else:
    ...
    def _format_file_content(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if not line:
                if (
                    started and not prev_empty
                ):  # 只有在开始添加内容后才添加空行 / Only add empty lines after content starts
    ...
    def _create_history_summary(self, history: str) -> str:
        ...
        for line in lines[:20]:  # 只取前20行 / Only take first 20 lines
            ...
            ):  # 标题或较长的内容行 / Headers or longer content lines
        ...
        if summary_lines:
            ...
            return (
                "## 历史参考信息 / Historical Reference Information\n\n"
                + history[:500]
                + "..."
            )
    ...
    def set_history_folder(self, folder_path: str) -> None:
        """
        设置历史参考文件夹路径 / Set history reference folder path

        Args:
            folder_path: 新的历史文件夹路径 / New history folder path
        """
        self.history_folder = Path(folder_path)
        self.logger.info(
            f"历史文件夹已切换到: {self.history_folder} / History folder switched to: {self.history_folder}"
        )

    def get_current_history_folder(self) -> str:
        """
        获取当前历史参考文件夹路径 / Get current history reference folder path

        Returns:
            str: 当前历史文件夹路径 / Current history folder path
        """
        return str(self.history_folder)

    def switch_to_prompt_history(self, prompt_name: str) -> bool:
        """
        切换到指定提示词对应的历史文件夹 / Switch to history folder for specified prompt
        ...
        """
        try:
            # 构建提示词对应的历史文件夹路径 / Build history folder path for prompt
            base_folder = (
                self.history_folder.parent
                if self.history_folder.name != "history_references"
                else self.history_folder.parent
            )
            prompt_history_folder = base_folder / "history_references" / prompt_name

            # 确保文件夹存在 / Ensure folder exists
            prompt_history_folder.mkdir(parents=True, exist_ok=True)

            # 切换到新文件夹 / Switch to new folder
            self.set_history_folder(str(prompt_history_folder))

            return True

        except Exception as e:
            self.logger.error(
                f"切换到提示词历史文件夹失败 / Failed to switch to prompt history folder: {e}"
            )
            return False
