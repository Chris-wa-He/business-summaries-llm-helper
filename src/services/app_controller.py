...
class AppController:
    ...
    def _initialize_app(self):
        ...
        try:
            ...
            self.system_prompt_manager = SystemPromptManager(
                self.config_manager, self.history_processor
            )
            ...
            raise CaseSummaryError(
                f"应用初始化失败: {e} / Application initialization failed: {e}"
            )
    ...
    def initialize_models(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            self.logger.info(
                f"模型初始化完成，共 {sum(len(models) for models in self.available_models.values())} 个模型 / Model initialization completed, {sum(len(models) for models in self.available_models.values())} models total"
            )
    ...
    def process_case_summary(
        self, case_input: str, model_id: str, custom_system_prompt: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            self.logger.info(
                f"开始处理案例总结，模型: {model_id} / Starting case summary processing, model: {model_id}"
            )
            ...
            if not self.model_manager.is_model_available(model_id):
                raise CaseSummaryError(
                    f"模型不可用: {model_id} / Model not available: {model_id}"
                )
            ...
            if custom_system_prompt:
                ...
                # 优先使用激活的系统提示词 / Prefer active system prompt
            ...
            raise CaseSummaryError(
                f"案例总结处理失败: {e} / Case summary processing failed: {e}"
            )
    ...
    # 系统提示词管理方法 / System prompt management methods

    def get_available_prompts(self) -> List[Dict[str, Any]]:
        """
        获取可用的系统提示词列表 / Get available system prompts list

        Returns:
            提示词列表 / Prompts list
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return []

        try:
            return self.system_prompt_manager.list_prompts()
        except Exception as e:
            self.logger.error(f"获取提示词列表失败: {e} / Failed to get prompts list: {e}")
            return []

    def list_prompts(self) -> List[Dict[str, Any]]:
        """
        获取可用的系统提示词列表 (别名方法) / Get available system prompts list (alias method)

        Returns:
            提示词列表 / Prompts list
        """
        return self.get_available_prompts()

    def get_active_prompt(self) -> Optional[Dict[str, str]]:
        """
        获取当前激活的系统提示词 / Get current active system prompt

        Returns:
            激活的提示词信息 / Active prompt information
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return None

        try:
            return self.system_prompt_manager.get_active_prompt()
        except Exception as e:
            self.logger.error(f"获取激活提示词失败: {e} / Failed to get active prompt: {e}")
            return None

    def get_prompt(self, name: str) -> Optional[str]:
        """
        获取指定名称的系统提示词内容 / Get system prompt content by name
        ...
        try:
            return self.system_prompt_manager.get_prompt(name)
        except Exception as e:
            self.logger.error(f"获取提示词内容失败: {e} / Failed to get prompt content: {e}")
            return None
    ...
    def set_active_prompt(self, prompt_name: str) -> bool:
        ...
        Returns:
            设置是否成功 / Whether setting was successful
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return False

        try:
            return self.system_prompt_manager.set_active_prompt(prompt_name)
        except Exception as e:
            self.logger.error(f"设置激活提示词失败: {e} / Failed to set active prompt: {e}")
            return False
    ...
    def create_prompt(self, name: str, content: str) -> bool:
        ...
        Returns:
            创建是否成功 / Whether creation was successful
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return False

        try:
            return self.system_prompt_manager.create_prompt(name, content)
        except Exception as e:
            self.logger.error(f"创建提示词失败: {e} / Failed to create prompt: {e}")
            return False
    ...
    def update_prompt(self, name: str, content: str) -> bool:
        ...
        Returns:
            更新是否成功 / Whether update was successful
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return False

        try:
            return self.system_prompt_manager.update_prompt(name, content)
        except Exception as e:
            self.logger.error(f"更新提示词失败: {e} / Failed to update prompt: {e}")
            return False
    ...
    def delete_prompt(self, name: str) -> bool:
        ...
        Returns:
            删除是否成功 / Whether deletion was successful
        """
        if not self.is_initialized or not self.system_prompt_manager:
            return False

        try:
            return self.system_prompt_manager.delete_prompt(name)
        except Exception as e:
            self.logger.error(f"删除提示词失败: {e} / Failed to delete prompt: {e}")
            return False
    ...
    def _load_history_reference(self, case_input: str) -> str:
        ...
        try:
            ...
            history_content = self.history_processor.process_history_content(
                history_files
            )

            # 筛选相关历史信息 / Filter relevant history information
            relevant_history = self.history_processor.filter_relevant_history(
                case_input, history_content
            )
            ...
            self.logger.error(
                f"加载历史参考信息失败: {e} / Failed to load history reference: {e}"
            )
            return ""

    def _generate_summary(
        self, model_id: str, user_prompt: str, system_prompt: str
    ) -> str:
        """
    ...
    def get_initialization_status(self) -> Tuple[bool, str]:
        ...
        if self.is_initialized:
            ...
            return (
                False,
                "应用未初始化或初始化失败 / Application not initialized or initialization failed",
            )
