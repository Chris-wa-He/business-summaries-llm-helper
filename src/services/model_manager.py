...
class ModelManager:
    ...
    def refresh_available_models(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            self._categorized_models = self.bedrock_client.filter_models_by_provider(models)
            
            # 为每个模型添加显示名称 / Add display name for each model
            for category, category_models in self._categorized_models.items():
                for model in category_models:
                    model['displayName'] = self.bedrock_client.get_model_display_name(model['modelId'])
            
            self.logger.info(f"成功刷新模型列表，共 {len(models)} 个模型 / Successfully refreshed model list, {len(models)} models total")
    ...
    def get_models_by_category(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if self._categorized_models is None:
            ...
            try:
                ...
                self.logger.warning(f"无法获取实时模型列表，使用配置文件默认值: {e} / Cannot get real-time model list, using config defaults: {e}")
                return self._get_fallback_models()
                ...
                for model_config in models:
                    if isinstance(model_config, dict) and 'id' in model_config:
                        fallback_models[category].append({
                            ...
                            'displayName': model_config.get('name', model_config['id']),
                            'modelName': model_config.get('name', model_config['id'])
                        })
    ...
    def validate_model_access(self, model_id: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"模型 {model_id} 访问验证失败: {e} / Model {model_id} access validation failed: {e}")
            return False
