...
class PromptBuilder:
    ...
    def build_prompt(self, 
        ...
        try:
            ...
            if formatted_history.strip():
                ...
                user_prompt_parts.append("Please generate a structured and professional case summary based on the above historical reference information and case content.")
            else:
                user_prompt_parts.append("请根据案例内容，生成一个结构化、专业的案例总结。")
                user_prompt_parts.append("Please generate a structured and professional case summary based on the case content.")
            ...
            self.logger.debug(f"构建的用户提示词长度: {len(user_prompt)} 字符 / Built user prompt length: {len(user_prompt)} characters")
    ...
    def _ensure_prompt_length(self, user_prompt: str, system_prompt: str) -> str:
        ...
        available_length = self.max_prompt_length - len(system_prompt) - 500  # 保留500字符缓冲 / Reserve 500 characters buffer
        
        if available_length < 1000:  # 如果可用长度太短 / If available length is too short
            self.logger.warning("系统提示词过长，可能影响用户输入 / System prompt too long, may affect user input")
            available_length = 1000
        
        # 智能截断用户提示词 / Intelligently truncate user prompt
        truncated_prompt = self._truncate_prompt_intelligently(user_prompt, available_length)
        
        self.logger.warning(f"提示词被截断，原长度: {len(user_prompt)}, 截断后: {len(truncated_prompt)} / Prompt truncated, original: {len(user_prompt)}, after: {len(truncated_prompt)}")
    ...
    def _truncate_history_intelligently(self, history: str) -> str:
        ...
        for line in lines:
            line = line.strip()
            if line.startswith('#') or '重要' in line or '关键' in line or 'important' in line.lower() or 'key' in line.lower():
        ...
        if len(truncated_lines) < len(lines):
            ...
            truncated_lines.append("... (历史信息已截断以适应长度限制) / (History truncated to fit length limit) ...")
    ...
    def _truncate_prompt_intelligently(self, prompt: str, max_length: int) -> str:
        ...
        for line in essential_lines:
            if current_length + len(line) + 1 > max_length - 100:  # 保留100字符缓冲 / Reserve 100 characters buffer
                break
            truncated_lines.append(line)
            current_length += len(line) + 1
...
请保持总结的客观性和专业性。"""
    ...
    def validate_prompt_components(self, case_input: str, system_prompt: str) -> bool:
        ...
        if len(case_input.strip()) < 10:
            self.logger.warning("案例输入内容过短，可能影响总结质量 / Case input too short, may affect summary quality")
        
        if not system_prompt or not system_prompt.strip():
            self.logger.warning("系统提示词为空，将使用默认提示词 / System prompt empty, will use default prompt")
