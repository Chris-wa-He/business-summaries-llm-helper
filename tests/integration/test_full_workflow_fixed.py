"""
完整工作流集成测试（修复版）/ Full Workflow Integration Tests (Fixed)
...
class TestFullWorkflowFixed:
    """完整工作流测试类（修复版）/ Full workflow test class (Fixed)"""
...
"""
    ...
    def test_complete_workflow_with_proper_mocking(self, mock_bedrock_client, mock_get_session, mock_validate_credentials):
        """测试完整工作流（正确的模拟）/ Test complete workflow with proper mocking"""
        # 设置mock / Setup mocks
        mock_validate_credentials.return_value = True
        mock_get_session.return_value = Mock()
        
        # 创建模拟的Bedrock客户端实例 / Create mocked Bedrock client instance
        mock_client_instance = Mock()
        mock_bedrock_client.return_value = mock_client_instance
        
        # 模拟模型列表方法 / Mock model list methods
...
"""
    ...
    def test_history_processing_integration(self, mock_validate_credentials):
        ...
        with patch('src.clients.bedrock_client.BedrockClient'):
            ...
            assert len(history_files) == 1  # 应该有1个测试文件 / Should have 1 test file
